...
import { X, Upload, CheckCircle, AlertCircle, Info, AlertTriangle } from 'lucide-react';
...
export function ImportBeneficiariesModal({ isOpen, onClose, onSuccess }: ImportBeneficiariesModalProps) {
    const [file, setFile] = useState<File | null>(null);
    const [isImporting, setIsImporting] = useState(false);
    const [summary, setSummary] = useState<BeneficiaryImportSummary | null>(null);
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
            <Card className="w-full max-w-lg shadow-2xl animate-in fade-in zoom-in duration-200">
                <div className="flex items-center justify-between p-4 border-b">
                    <h2 className="text-xl font-bold text-gray-800">Import Beneficiaries</h2>
                        ...
                        <div className="space-y-6">
                            <div className="bg-blue-50 border border-blue-100 p-4 rounded-xl flex gap-3 text-sm text-blue-700">
                                ...
                                <div>
                                    ...
                                    <ul className="list-disc list-inside space-y-1">
                                        ...
                                        <li>Required columns: <strong>NAME, AGE, GENDER, DATE OF REGISTRATION, TYPE OF BENEFICIARY, STATUS, COUNTRY, STATE, PURPOSE OF VISIT, DISABILITY TYPE, PROGRAM</strong></li>
                                        <li>Download the template below to get the correct format.</li>
                                        <li>Records with duplicate mobile numbers are automatically skipped.</li>
                                    </ul>
                                </div>
                            </div>

                            <div
                                className={`border-2 border-dashed rounded-xl p-8 flex flex-col items-center justify-center transition-colors ${file ? 'border-primary bg-primary/5' : 'border-gray-300 hover:border-primary'}`}
                            ...
                            >
                                ...
                                <Button
                                    variant="outline"
                                    className="mt-4"
                                    onClick={() => document.getElementById('beneficiary-import-upload')?.click()}
                                >
                                    Select File
                                </Button>
                            </div>
                            ...
                            <div className="flex justify-between items-center pt-2">
                                <button
                                    onClick={(e) => { e.preventDefault(); downloadBeneficiaryImportTemplate(); }}
                        ...
                        <div className="space-y-6">
                            <div className="flex flex-col items-center text-center">
                                ...
                                <p className="text-sm text-text-muted">
                                    {summary.imported > 0
                                        ? `${summary.imported} beneficiar${summary.imported === 1 ? 'y' : 'ies'} saved and will sync when online.`
                            ...
                            {summary.errorDetails.length > 0 && (
                                <div className="bg-red-50 border border-red-100 rounded-xl p-4 max-h-32 overflow-y-auto">
                                    <p className="text-xs font-bold text-red-700 uppercase tracking-wider mb-2">Error Details</p>
                                    {summary.errorDetails.map((detail, i) => (
